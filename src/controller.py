...
class Controller:
    def extract_uuid4s(self, path: str, fps: int = 1) -> List[str]:
        ...
        if os.path.isfile(path):
            if path.lower().endswith((".png", ".jpg", ".jpeg")):
                ...
                raise ValueError(
                    "Unsupported file format. Please provide an image or video file."
                )
        else:
    ...
    def extract_verify_uuid4s(self, path: str, fps: int = 1) -> List[Tuple[str, bool]]:
        ...
        unique_validated_uuid4s = list(
            {uuid: (uuid, is_valid) for uuid, is_valid in validated_uuid4s}.values()
        )
